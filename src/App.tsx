...
function App() {
  ...
  const amplitudeScale =
    customConfig.presets[activePresetIdx].angle / BASE_ANGLE;
  ...
  const movementRepeatTimerRef = useRef<ReturnType<typeof setTimeout> | null>(
    null,
  );
  useEffect(() => {
    ...
    if (phase !== "guide") {
      ...
      if (movementRepeatTimerRef.current) {
        clearTimeout(movementRepeatTimerRef.current);
    ...
    const value = Math.abs(
      headRotation[activeStep.axis as keyof typeof headRotation],
    );
  ...
  useEffect(() => {
    if (
      ...
      if (returnCueTimerRef.current) {
        clearTimeout(returnCueTimerRef.current);
        returnCueTimerRef.current = null;
      }
      return;
    }
    if (!inHoldZone) {
  ...
  useEffect(() => {
    ...
    if (customConfig.voiceCuesEnabled)
      setTimeout(() => playCue(-3, lang, 2), 800);
